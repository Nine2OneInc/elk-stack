...
poll_ready elasticsearch 'http://localhost:9200/' 'elastic:eleclara'

log 'Waiting for Kibana readiness'
poll_ready kibana 'http://localhost:5601/api/status' 'kibana:eleclara'
...
response="$(curl 'http://localhost:5601/api/saved_objects/_find?type=index-pattern' -u elastic:eleclara)"
echo "$response"
...
sleep 1
curl -X POST 'http://localhost:9200/_refresh' -u elastic:eleclara \
	-s -w '\n'

log 'Searching message in Elasticsearch'
response="$(curl 'http://localhost:9200/_count?q=message:dockerelk&pretty' -u elastic:eleclara)"
echo "$response"
